...
4. **Validate loop — tiered.** `validateConfig` is remote (uploads the bundle to the tenant API; needs `tenantLocator` + PAT in `settings.gradle.kts`) — **detect credentials before invoking gradle** and follow the sibling skill's "Validation without tenant credentials" section (`../socotra-config-from-xlsx/SKILL.md`): always run the local lint (`../socotra-config-from-xlsx/scripts/lint_config.py socotra-config`); run `./gradlew validateConfig` only when credentials exist; with no credentials, deliver after a clean lint and state plainly that remote validation is still required before deploy. Never ask for a PAT in chat; never hardcode one into a committed file. Deployment (`createTenant`/`deployConfig`) is the **user's** to run.
